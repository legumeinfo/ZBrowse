...
macroSyntenyBlocksMicroservice <- function(url, families, matched, intermediate, mask, targets = NULL, chr1name = "", org1 = "", org2 = "") {
  pf <- list(chromosome = families, matched = unbox(matched), intermediate = unbox(intermediate), mask = unbox(mask), optionalMetrics = "levenshtein")
  ...
  bb2 <- do.call(rbind, lapply(results$blocks, function(b) {
    do.call(rbind, lapply(b$blocks, function(bi) {
      ...
      data.frame(chromosome = trailingInteger(b$chromosome), i = bi$i + 1, j = bi$j + 1,
        fmin = bi$fmin, fmax = bi$fmax, orientation = bi$orientation, levenshtein = bi$optionalMetrics[[1]],
